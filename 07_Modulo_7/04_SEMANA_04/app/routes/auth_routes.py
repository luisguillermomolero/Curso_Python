...
def register(user: UserCreate, db: Session = Depends(get_db)):
    if len(user.username) < 4 or len(user.username) > 20:
        raise HTTPException(status_code=400, detail="El nombre del usuario debe estar entre 4 y 20 caracteres")
    if len(user.password) < 6:
        raise HTTPException(status_code=400, detail="La contraseña debe ser superior a 6 caracteres")
