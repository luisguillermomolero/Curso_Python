...
def create_user(user: UserCreate, db: Session):
    hashed_pw = pwd_contex.hash(user.password)
...
def authenticate_user(username: str, password: str, db: Session):
    ...
    if not user or not pwd_contex.verify(password, user.hashed_password):
        return None
    return user
...
def get_current_user(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    try:
        ...
        if user is None:
            raise HTTPException(status_code=403, detail="Usuario noencontrado")
        return user
    except JWSError:
